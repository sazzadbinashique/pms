        ...
        <div class="card mb-3">
            <div class="card-header">
                <ul class="nav nav-pills card-header-pills justify-content-between">
                    <li class="nav-item" style="font-family:arial;font-weight:bold;">Product Category List</li>
                    @can('product-category-create')
                        <li class="nav-item"><a class="btn btn-success text-white btn-sm" href="{{route('medicineCategories.create')}}">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped" id="medicine_category_tbl">
                        ...
                        <tbody>
                        @foreach($medicineCategories as $key=>$data)
                            @php
                            ...
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    <div class="d-grid gap-2 d-md-flex justify-content-center">
                                        <a class="btn btn-info text-white btn-sm" href="{{ route('medicineCategories.show',$data->id) }}">Details</a>

                                        @can('product-category-edit')
                                        <a class="btn btn-primary btn-sm" href="{{ route('medicineCategories.edit',$data->id) }}">Edit</a>
                                        @endcan

                                        @can('product-category-delete')
                                        <form action="{{ route('medicineCategories.destroy', $data->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
...
@section('third_party_scripts')
    ...
    let url = "{{ route('medicineCategories.index') }}";
    $('#medicine_category_tbl').DataTable( {
