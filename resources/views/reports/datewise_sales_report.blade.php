        ...
        <div class="card mb-3">
            <div class="card-header">
                <ul class="nav nav-pills card-header-pills justify-content-between">
                    <li class="nav-item" style="font-family:arial;font-weight:bold;"><i class="fa fa-registered" style="font-size:14px;color:black">&nbsp;</i>Date Wise Sales Report</li>
                </ul>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ url('/search_datewise_sales_report') }}" method="GET">
                <div class="row">
              ...
              <span style="font-size:14px;color:blue;">@if($searchchk==1)search completed. please check data table @endif<span>
                <?php $sub_total_sales = 0; $sub_total_discount = 0; $sub_total_payable = 0; ?>
              <input type="hidden" id="base" value="{{ $base_name }}">
              <input type="hidden" id="from_dt" value="{{ $from_date }}">
              <input type="hidden" id="to_dt" value=" {{ $to_date }}" >
              @foreach($datewise_sales_report as $key=> $ddd)
              <?php $sub_total_sales = $sub_total_sales + $ddd->total_sales_amount ?>
              <?php $sub_total_discount = $sub_total_discount + $ddd->total_discount_amnt ?>
              <?php $sub_total_payable = $sub_total_payable + $ddd->total_payable_amount ?>
              @endforeach
              <input type="hidden" id="sub_total_sales" value=" {{ $sub_total_sales }}" >
              <input type="hidden" id="sub_total_discount" value=" {{ $sub_total_discount }}" >
              <input type="hidden" id="sub_total_payable" value=" {{ $sub_total_payable }}" >
              </br></br>
          

                <div class="table-responsive">
                    <table class="table table-hover table-striped" id="datewise_sales_rpt_tbl">
                        ...
                        <tbody>
                          @foreach($datewise_sales_report as $key=> $data)
                          <tr>
                          <td>{{ ++$key }}</td>                     
                          <td>{{ $data->base_name }}</a></td>
...
@section('third_party_scripts')
    ...
    <script>
      ...
      let url = "{{ route('datewise_sales_report') }}";
      ...
      var to_dt =  $('#to_dt').val();
      var sub_total_sales = $('#sub_total_sales').val();
      var sub_total_discount = $('#sub_total_discount').val();
      var sub_total_payable = $('#sub_total_payable').val();
      ...
      $('#datewise_sales_rpt_tbl').DataTable( {
      
            dom: 'Bfrtip',
            buttons: [
                      { extend : 'copy', filename: 'Date Wise Sales Report' },
                      { extend : 'csv', filename: 'Date Wise Sales Report' },
                      { extend : 'excel', filename: 'Date Wise Sales Report',
                                  title:'Report : ' + '\n' + 'Date Wise Sales, ' + ' Base : ' + base + ' , ' + ' From Date : ' + from_dt + ' , ' +' To Date : ' + to_dt + ' , ' +' Sub Total Sales : ' + sub_total_sales + ' , ' +' Sub Total Discount : ' + sub_total_discount + ' , ' +' Sub Total Payable : ' + sub_total_payable
                      },
                      { extend : 'pdf', orientation: '', pageSize: 'LEGAL', filename: 'Date Wise Sales Report',
                                  ...
                                  'From Date : ' + from_dt + '\n' +
                                  'To Date : ' + to_dt + '\n' +
                                  'Total Payable : ' + sub_total_payable + ' Tk '
                      },
                      { extend : 'print', title:'Date Wise Sales Report' },
            ],    
            'pageLength': 10
