        ...
        <div class="card mb-3">
            <div class="card-header">
                <ul class="nav nav-pills card-header-pills justify-content-between">
                    <li class="nav-item" style="font-family:arial;font-weight:bold;"><i class="fa fa-registered" style="font-size:14px;color:black">&nbsp;</i>Stock Available Products Report</li>
                </ul>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ url('/search_stock_available') }}" method="GET">
                <div class="row">
                  ...
                  <div class="col-xs-2 col-sm-2 col-md-2">         
                    <select class="form-control" name="product_group" id="product_group" style="background-color:#f4f6f9; border:1px solid #4B92f9;"> 
                        ...
                        @foreach ($stock_available_prod_grp as $stock_available_prod_grp)
                        <option value="{{ $stock_available_prod_grp->product_group }}">{{ $stock_available_prod_grp->product_group }}</option>				  
                        @endforeach
                    </select>               
                  </div>
                  ...
                  <div class="col-xs-2 col-sm-2 col-md-2">         
                    <select class="form-control" name="product_category" id="product_category" style="background-color:#f4f6f9; border:1px solid #4B92f9;">
                        ...
                        @foreach ($stock_available_prod_catg as $stock_available_prod_catg)
                        <option value="{{ $stock_available_prod_catg->product_category }}">{{ $stock_available_prod_catg->product_category }}</option>				  
                        @endforeach
                    </select>                
                  </div>
                  ...
                  <div class="col-xs-2 col-sm-2 col-md-2">         
                    <select class="form-control" name="medicine_name" id="medicine_name" style="background-color:#f4f6f9; border:1px solid #4B92f9;">
                        ...
                        @foreach ($stock_available_prod_name as $stock_available_prod_name)
                        <option value="{{ $stock_available_prod_name->medicine_name }}">{{ $stock_available_prod_name->medicine_name }}</option>				  
                        @endforeach
                    </select>                
                  </div>
                  ...
                  <div class="col-xs-2 col-sm-2 col-md-2">         
                    <select class="form-control" name="generic_name" id="generic_name" style="background-color:#f4f6f9; border:1px solid #4B92f9;">
                        ...
                        @foreach ($stock_available_prod_generic_name as $stock_available_prod_generic_name)
                        <option value="{{ $stock_available_prod_generic_name->generic_name }}">{{ $stock_available_prod_generic_name->generic_name }}</option>				  
                        @endforeach
                    </select>                
                  </div>
              ...
              <span style="font-size:14px;color:blue;">@if($searchchk==1)search completed. please check data table @endif<span>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped" id="stock_avialable_rpt_tbl">
                        ...
                        <tbody>
                          @foreach($stock_available_products as $key=> $data)
                          <tr>
                          <td>{{ ++$key }}</td>                     
...
@section('third_party_scripts')
    ...
    <script>
      ...
      let url = "{{ route('stock_available_report') }}";
      ...
      $('#stock_avialable_rpt_tbl').DataTable( {
      
            dom: 'Bfrtip',
            buttons: [
                      { extend : 'copy', filename: 'Stock Available Products Report' },
                      { extend : 'csv', filename: 'Stock Available Products Report' },
                      { extend : 'excel', filename: 'Stock Available Products Report',
                                  title:'Report, ' + '\n' + ' Stock Available Products, ' + '\n' +' Date :' + formateDt
                      },
                      { extend : 'pdf', orientation: '', pageSize: 'LEGAL', filename: 'Stock   Available Products Report',
                      ...
                      { extend : 'print', title:'Stock Available Products Report' },
            ],    
            'pageLength': 10
