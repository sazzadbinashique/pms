        ...
        <div class="card mb-3">
            <div class="card-header">
                <ul class="nav nav-pills card-header-pills justify-content-between">
                    <li class="nav-item" style="font-family:arial;font-weight:bold;"><i class="fa fa-registered" style="font-size:14px;color:black">&nbsp;</i>Date Wise Purchase Report</li>
                </ul>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ url('/search_datewise_purchase_report') }}" method="GET">
                <div class="row">
              ...
              @foreach($datewise_purchase_report as $key=> $ddd)
              <?php $sub_total_purchase = $sub_total_purchase + $ddd->purchase_price_total ?>
              @endforeach
              <input type="hidden" id="sub_total_purchase" value=" {{ $sub_total_purchase }}" >

              </br></br>
          

                <div class="table-responsive">
                    <table class="table table-hover table-striped" id="datewise_purchase_rpt_tbl">
                        ...
                        <tbody>
                          @foreach($datewise_purchase_report as $key=> $data)
                          <tr>
                          <td>{{ ++$key }}</td>                     
                          <td>{{ $data->base_name }}</a></td>
                          <td>{{ $data->medicine_name }} {{ $data->strength }}</td>
...
@section('third_party_scripts')
    ...
    <script>
      ...
      let url = "{{ route('datewise_purchase_report') }}";
      ...
      var to_dt =  $('#to_dt').val();
      var sub_total_purchase = $('#sub_total_purchase').val();
      ...
      $('#datewise_purchase_rpt_tbl').DataTable( {
      
            dom: 'Bfrtip',
            buttons: [
                      { extend : 'copy', filename: 'Date Wise Purchase Report' },
                      { extend : 'csv', filename: 'Date Wise Purchase Report' },
                      { extend : 'excel', filename: 'Date Wise Purchase Report',
                                  title:'Report : ' + '\n' + 'Date Purchase Purchase, ' + ' Base : ' + base + ' , ' + ' From Date : ' + from_dt + ' , ' +' To Date : ' + to_dt + ' , ' +' Sub Total Purchase : ' + sub_total_purchase
                      },
                      { extend : 'pdf', orientation: 'landscape', pageSize: 'LEGAL', filename: 'Date Wise Purchase Report',
                                  ...
                                  'From Date : ' + from_dt + '\n' +
                                  'To Date : ' + to_dt + '\n' +
                                  'Sub Total Purchase : ' + sub_total_purchase + ' Tk '
                      },
                      { extend : 'print', title:'Date Wise Purchase Report' },
            ],    
            'pageLength': 10
